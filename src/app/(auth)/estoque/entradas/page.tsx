...
const forns = getLS<any[]>("fornecedores", []);
...
const r = lista.filter((e) => {
    const mBusca = !filtros.busca ||
    e.produto.toLowerCase().includes(filtros.busca.toLowerCase()) ||
    e.notaFiscal.toLowerCase().includes(filtros.busca.toLowerCase());
...
<div className="w-full min-h-screen bg-secondary p-4 md:p-8 flex flex-col items-center">
    ...
    <div className="bg-primary rounded-2xl border border-white/10 p-6 space-y-5">
        ...
        <Field label="Produto ou NF-e">
            ...
            placeholder="Buscar por produto ou NF" className={inputClass} />
        </Field>
        ...
        {resultado.length === 0 ? (
            ...
            <table className="w-full text-sm">
                ...
                {resultado.map((entrada) => (
                    ...
                    <td className="px-4 py-4 text-white/70">{entrada.fornecedor}</td>
    ...
    {/* MODAL FORM (Nova Entrada / Editar) */}
    {modalForm && (
